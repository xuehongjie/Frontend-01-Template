...
class BinaryHeap {
  ...
  take() {
    ...
    while (i < this.data.length) {
      let currentIndex = i * 2 + 1;
      let nextIndex = i * 2 + 2;
      if (currentIndex >= this.data.length) {
        break;
      }
      if (nextIndex >= this.data.length) {
        this.data[i] = this.data[currentIndex];
        i = currentIndex;
        break;
      }
      if (this.compare(this.data[currentIndex], this.data[nextIndex]) < 0) {
        this.data[i] = this.data[currentIndex];
        i = currentIndex;
      } else {
        this.data[i] = this.data[nextIndex];
        i = nextIndex;
      }
    }
...
class Path {
  ...
  async find() {
    ...
    this.$container.children[start[1] * 100 + start[0]].style.backgroundColor =
      'green';
    this.$container.children[end[1] * 100 + end[0]].style.backgroundColor =
      'red';
